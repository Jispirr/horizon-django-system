...
def seed_cars(apps, schema_editor):
    ...
    Brand = apps.get_model("horizon", "Brand")
    Category = apps.get_model("horizon", "Category")
    ...
    for data in CARS:
        ...
        # Map seed slug to DB slug where they differ
        brand_slug_map = {"benz": "mercedes-benz"}
        brand_obj = Brand.objects.filter(slug=brand_slug_map.get(brand_slug, brand_slug)).first()
